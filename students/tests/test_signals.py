...
class StudentSignalsTests(TestCase):
    ...
    def test_log_student_updated_added_event(self):
        ...
        out.seek(0)
        log_message = 'Student added: Demo Student (ID: %d)\n' % student.id
        self.assertEqual(out.readlines()[-1], log_message)

        # now update existing student and check last line in out
        student.ticket = '12345'
        student.save()
        out.seek(0)
        log_message = 'Student updated: Demo Student (ID: %d)\n' % student.id
        self.assertEqual(out.readlines()[-1], log_message)
    ...
    def test_log_student_deleted_event(self):
        ...
        student.delete()
        out.seek(0)
        log_message = 'Student deleted: Demo Student (ID: %d)\n' % sid
        self.assertEqual(out.readlines()[-1], log_message)
